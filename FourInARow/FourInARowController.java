...
public class FourInARowController {
    ...
    void ShowTheWinner(){
        ...
        showWinner.initStyle(StageStyle.UTILITY);
        showWinner.setTitle("End Of The Game");
        showWinner.setHeaderText("Congratulations");
        showWinner.setContentText("The " + game.turn + " player is the winner !!!");
    ...
    void ClickForCircle(ActionEvent event) {
        int freeBox = -1;//indicates where is the next free box to draw in
        double []point = new double[3];// point[0] = x,point[1] = y,point[2] = winner
        if (gameOn) {
            if (event.getSource() == button1) {
                freeBox = game.checkFreeBox(0);//check if there is a free box to draw in
                if (freeBox != -1)//if there is a free box calculate where to draw the circle
